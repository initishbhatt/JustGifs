...
class SearchAdapter : RecyclerView.Adapter<SearchAdapter.ViewHolder>() {
    private var searchedGifs: List<SearchGifs> = ArrayList()
    ...
    fun showGifs(searchedGifs: List<SearchGifs>?) {
    ...
    inner class ViewHolder(root: View) : RecyclerView.ViewHolder(root) {
        fun bind(searchedGifs: SearchGifs) = with(itemView) {
           // gif_name.text = searchedGifs.title
            //GlideApp.with(context).load(searchedGifs.images.fixedHeightStill.url).into(gif_imview)
        }
    }
}